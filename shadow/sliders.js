$(document).ready(function () {
    ...
    var useBacklightElem = $('#useBacklightSelect');
    initCheckbox(useBacklight, useBacklightElem);
	
    var useAOElem = $('#useAOSelect');
    initCheckbox(useAO, useAOElem);
    ...
    colorPickerName = "#backLightCol.colorPicker";
    colorString = color2hex(backLightCol);
    $(colorPickerName).attr("value", colorString);
    ...
    $(colorPickerName).minicolors({
        ...
        change: function(value) {
            ...
            if( typeof console === 'object' ) {
                var rgbObject = $(this).minicolors('rgbObject');
                backLightCol[0] =rgbObject.r / 255;
                backLightCol[1] =rgbObject.g / 255;
                backLightCol[2] =rgbObject.b / 255;
            }
        },
    });
    ...
    var EnvL_slider = $("#EnvL_slider");
    var EnvL_val = $("#EnvL_val");
    EnvL_slider.attr("data-slider-min", 0).attr("data-slider-max", 1).attr("data-slider-step", 0.01).attr("data-slider-value", EnvL).attr("data-slider-tooltip","hide").slider({});
    bindSliderValParam (EnvL_slider, EnvL_val, "EnvL");
    ...
    var clipN_slider = $("#clipN_slider");
    var clipN_val = $("#clipN_val");
    clipN_slider.attr("data-slider-min", 0).attr("data-slider-max", 1.0).attr("data-slider-step", 0.1).attr("data-slider-value", clipNum).attr("data-slider-tooltip","hide").slider({});
    bindSliderValParam (clipN_slider, clipN_val, "clipNum");	
	
    var biasN_slider = $("#biasN_slider");
    var biasN_val = $("#biasN_val");
    biasN_slider.attr("data-slider-min", 0).attr("data-slider-max", 10.0).attr("data-slider-step", 0.1).attr("data-slider-value", biasNum).attr("data-slider-tooltip","hide").slider({});
    bindSliderValParam (biasN_slider, biasN_val, "biasNum");
	
    var radiusN_slider = $("#radiusN_slider");
    var radiusN_val = $("#radiusN_val");
    radiusN_slider.attr("data-slider-min", 0).attr("data-slider-max", 10.0).attr("data-slider-step", 0.1).attr("data-slider-value", radiusNum).attr("data-slider-tooltip","hide").slider({});
    bindSliderValParam (radiusN_slider, radiusN_val, "radiusNum");
