...
function main() {
  ...
  const fsSource = /*glsl */ `
    ...
    void initialize(){
        ...
        spheres[0].color = vec3(1.0, 0.0, 0.0); // red

        planes[0].point = vec3 (-1.0, 1.0, 0.0);
        planes[0].normal = normalize(vec3 (1.0, 0.0, 0.0));
        planes[0].color = vec3(0, 0, 1.0); // blue
    ...
    vec3 checkIntersectPlane(Plane plane, Ray ray, Light light){

        vec3 p_i = plane.point;
        vec3 n_i = plane.normal;
        vec3 colorOfPlane = plane.color;
        vec3 p_e = ray.origin;
        vec3 n_pe = ray.direction;
        vec3 lightSource = light.position;
        float ambience = light.ambience;
        vec3 color = vec3(0.0, 0.0, 0.0);

        // find point if ray that hits the plane
        vec3 p_h;
  
        // if negative then do not color
        float numer = dot(n_i, p_e - p_i);
        if (numer < 0.0)
            return color;
  
        // calculate the point on the ray that hits the plane
        float denom = dot(n_i, n_pe);

        // if denom is positive then do not color
        if (denom >= 0.0)
            return color;

        // calculate location on plane and color of plane
        float t_h = -1.0 * numer / denom;
        // vec3 p_h = p_e + (t_h*n_pe);
        color = colorOfPlane*(ambience + ((1.0-ambience)*max(0.0,dot(n_i, lightSource))));
    ...
    void main(){
        ...
        // vec3 color = checkIntersectPlane(planes[0], rays[0], light[0]);
