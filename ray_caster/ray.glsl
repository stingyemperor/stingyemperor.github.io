...
bool intersectMesh(in Mesh mesh) {
...
vec3 phongShading(vec3 N, vec3 L, vec3 V, bool shadow, Light light, Material material) {
    ...
    for(int i = 0; i < 5; i++) {
        if(intersect(pos, dir, info)) {
            ...
            if(info.reflectType == 5) {
                ...
                c = phongShading(N, L, V, false, light, sphere);
            }
            ...
            if(info.reflectType == 0) { // ground
                ...
                c = phongShading(N, L, V, shadow, light, plane);
                ...
                Material sphere = sphere(uLambertianColor);
                c = phongShading(N, L, V, shadow, light, sphere);
                ...
                c = phongShading(N, L, V, shadow, light, mirror);
