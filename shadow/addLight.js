...
mouseXYenv = [Math.random()-0.5, Math.random()-0.5];
var canvas = document.getElementById( "gl-canvas" );
    document.addEventListener("mousemove", function(evt){
        if(mouseFlag === 1){
            
            setMouseEnvPos(canvas, evt);
        }
    }, false);
    document.addEventListener("touchmove", function(evt){
        if(mouseFlag === 1){
...
function setMouseEnvPos(canvas, evt){
    var rect = canvas.getBoundingClientRect();

       
    mouseXYenv[0] = getMousePos(canvas, evt).x;
    mouseXYenv[1] = getMousePos(canvas, evt).y;
    //setLightMarkPosition(i);
    console.log(mouseXYenv);
